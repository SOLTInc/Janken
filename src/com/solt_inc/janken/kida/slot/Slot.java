...
public class Slot {
	...
	public void playSlot() {
		...
		Util ユーティリティ = new Util();
		スロットユーティリティ スロ = new スロットユーティリティ();

		boolean ループさせます = true;

		while (ループさせます) {

			System.out.println("現在のコイン枚数:" + Bean.getsCoin() + "枚");
			System.out.println("コインを賭けて下さい（最大10枚）   98:配当表  99:終了");
			// System.out.println("コインを何枚賭けますか？   現在のコイン枚数:"+Bean.getsCoin()+"枚");
			// System.out.println("1:1枚  2:5枚  3:10枚  98:配当表  99:終了");
			System.out.println("");
			...
			while (!コイン入力チェック) {
				...
				if (コイン枚数 == 99) {
					...
					if (Bean.getsCoin() - コイン枚数 <= 0) {
						System.out.println("現在のコイン枚数:" + Bean.getsCoin() + "枚");
						System.out
								.println("コインが足りません もう一度賭けて下さい（最大10枚）   98:配当表  99:終了");
						System.out.println("");
						コイン入力チェック = false;
					} else {
						// 足りている場合、コインを引く
